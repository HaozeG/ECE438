...
int main(int argc, char *argv[])
{
	...
	while(1) {  // main accept() loop
		...
		if (!fork()) { // this is the child process
			...
			// get path_to_file
			if ((path_to_file = strstr(buf, "GET /")) == NULL) {
				// bad request
			...
			path_to_file += 5;
			if (strtok(path_to_file, " HTTP") == NULL) {
				// bad request
				sprintf(buf, "HTTP/1.1 400 Bad Request\r\n\r\n");
				if (send(new_fd, buf, strlen(buf), 0) == -1)
					perror("send");
			...
			// start file transmission if exists
			if ((fd = fopen(path_to_file, "rb")) == NULL) {
				// file not found
				...
				while ((numbytes = fread(buf, sizeof(char), BUFFER_SIZE - 1, fd)) > 0) {
